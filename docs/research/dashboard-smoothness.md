...
1. Secondary feeds (equity options Yahoo chains, options tape, alpaca) mounted with rankings and competed on cold start.
...
4. Dense asset grids used `backdrop-filter: blur(28px)` on dozens of cards — expensive paint.
...
| Area | Change |
|------|--------|
| Dashboard order | Keep glass desk (paper + top picks); asset grid next; defer tape / equity / radar / alpaca after idle |
| Paper agent | Poll with `tick=false`; one idle tick after mount |
| Gate | React Query cache; keep UI when already granted; preserve local `REQUIRE_LOGIN` pass-through |
| Cards | `React.memo` + `surface-dense` (no backdrop blur) |
