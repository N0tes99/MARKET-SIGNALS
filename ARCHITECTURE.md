...
| Surface 4 Runner Detection | `backend/app/engines/runner_engine/` | **Phase 6 13F (PIT 8-K + lagged quarterlies + 13F search)** |
...
| System | Path | Status |
|--------|------|--------|
| Runner Detection Engine | `backend/app/engines/runner_engine/` | **Phase 6 13F** |
...
| `/api/v1/runners/backtest` | GET | Structure-tape lead-time study + dated 8-K / lagged quarterlies / 13F search (no live Yahoo `info`) |
| `/api/v1/runners/tune` | GET | OOS structure-threshold grid vs baseline (not applied to live) |
...
**Status:** `PHASE 6 13F` — Stage 0–7 + EARLY / IGNITION / RUNNING lists emit when Yahoo fundamentals exist (structure-only still capped). Alert gates (`early` / `high`) fire Discord after a silent baseline. `/radar` table shows Opp vs Risk plus a stage rail. Lead-time study (`GET /api/v1/runners/backtest`) replays truncated daily tape plus **dated 8-K filing dates**, **lagged Yahoo quarterlies**, and **13F EDGAR search** by filing date (not a complete manager universe); live Yahoo `info` is not written into the past. OOS structure-accumulation tune (`GET /api/v1/runners/tune`) holds out famous pattern-study names and does **not** change live Radar (still 55). Modifier-weight search waits on dated discovery/SI. Paid SI vendor and 8-K beat/guidance NLP still open. See `docs/research/10x-runner-detection-layer.md`.
...
| M10 — Surface 4 Runner Detection | Fundamental inflection radar, discovery gap, stages, 10X Radar UI, lead-time backtests | **Phase 6 13F** |
