...
_CATALYST_FORMS = frozenset({"8-K", "8-K/A", "6-K"})
...
def _ticker_map() -> dict[str, str]:
    def _load() -> dict[str, str]:
        client = shared_client(
            timeout=12.0, name="sec-tickers", headers=_headers()
        )
...
def _parse_filings(
    ...
    for form, raw_date in zip(forms, dates, strict=False):
        if str(form) not in _CATALYST_FORMS:
            continue
        try:
    ...
    *,
    ...
    def _load() -> EdgarSnapshot:
        ...
        try:
            ...
            count, form, filed = _parse_filings(
                response.json(), today=now, window=window_days
            )
